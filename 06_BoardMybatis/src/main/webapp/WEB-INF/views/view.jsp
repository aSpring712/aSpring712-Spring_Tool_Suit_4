...
<script>
	...
	$("#commentBtn").click(function() {
		...
		$.ajax({
			type : "post",
			url : "/myapp06/reply/commentInsert", // 댓글 관련은 전부 reply를 타고 넘어가도록
	...
	$("#btnDelete").click(function() {
		...
		$.ajax({ // '확인' 클릭 시 비동기로 처리 후
			...
			url : "/myapp06/delete/${board.num}", // delete/num으로 이동 -> 현재 /로 시작하는 절대경로 사용함(프로젝트를 타고 감)
			success : function(resp) {
				/* alert(resp) */
				if (resp = "success") { // 삭제 성공 시 
					alert("삭제성공"); // 성공 메세지 띄워주고
					location.href = "/myapp06/list"; // list로 보냄 -> 그냥 list라고 적으면 view/list로 가짐! 상위(myapp06)로 이동하고 list로 가야하는데 절대경로로 써주면 편함 
				}
			} // success
		}) // ajax
	}) // btnDelete
