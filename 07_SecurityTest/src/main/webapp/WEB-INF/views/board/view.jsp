...
<script>
	...
	$("#commentBtn").click(function() {
		if(${empty sessionScope.sMember}) {
			alert("로그인하세요")
			location.href="/myapp07/member/login";
			return;
		}
		if($("#msg").val() == "") {
			alert("댓글을 적으세요");
		...
		$.ajax({
			type : "post",
			url : "/myapp07/reply/commentInsert", // 댓글 관련은 전부 reply를 타고 넘어가도록
	...
	$("#btnUpdate").click(function() {
		if (!confirm('정말 수정할까요?')) // '취소' 클릭 시
			return false; // 넘기지 않음
		location.href = "/myapp06/update/${board.num}"; // callback 함수 아님
	})
	...
	$("#btnDelete").click(function() {
		...
		$.ajax({ // '확인' 클릭 시 비동기로 처리 후
			...
			url : "/myapp07/delete/${board.num}", // delete/num으로 이동 -> 현재 /로 시작하는 절대경로 사용함(프로젝트를 타고 감)
			success : function(resp) {
				/* alert(resp) */
				if (resp = "success") { // 삭제 성공 시 
					alert("삭제성공"); // 성공 메세지 띄워주고
					location.href = "/myapp07/list"; // list로 보냄 -> 그냥 list라고 적으면 view/list로 가짐! 상위(myapp06)로 이동하고 list로 가야하는데 절대경로로 써주면 편함 
				}
			} // success
		}) // ajax
	}) // btnDelete
